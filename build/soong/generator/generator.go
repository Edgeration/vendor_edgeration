...
// Copyright (C) 2018 The edgerationOS Project
//
...
func init() {
	android.RegisterModuleType("edgeration_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = edgerationExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = edgerationExpandVariables(ctx, dep_file)
	...
	cmd := edgerationExpandVariables(ctx, String(g.properties.Cmd))
